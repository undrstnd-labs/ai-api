...
@router.post("/v1/rag/chat/completions", dependencies=[Depends(retrieve_api_key)])
...
@router.post("/v1/rag/completions", dependencies=[Depends(retrieve_api_key)])
async def rag_completions(
    request: CompletionRequestRAG, api_token=Depends(retrieve_api_key)
):
    try:
        model, api_key, base_url = await get_api_token_model_inference(
            api_token, request.model
